...
function Navbar(){
    ...
    const {totalCartItems, setIsOpen, isOpen, isLogin, setIsLogin} = useAppContext()
            ...
            <Container>
                <div className="flex flex-row-reverse justify-between">
                    <ul className="flex">
                        ...
                        <li>
                            {
                                isLogin ? <button className="ml-5 cursor-pointer" onClick={() => setIsLogin(false)} >Logout</button>:
                                <button className="ml-5 cursor-pointer" onClick={() => setIsOpen(true)} >user</button>  
                            }
                            {isOpen ? <Modal />:<></>}
                        </li>
                    </ul>
